...
app.post('/users', function(req, res) {
	...
	if (functionName=="register") {
		...
		var adminSecret = req.body.adminSecret;
		var orgName = req.body.orgName; //admin密码要与organization的ca对应
		var newUser = req.body.newUser;
		logger.debug('End point : /register');
		logger.debug('admin name : ' + adminName);
		logger.debug('Org name  : ' + orgName);
		logger.debug('admin Secret  : ' + adminSecret);
		if (!adminName) {
		...
		//use admin to register new user
		helper.registerUsers(adminName,adminSecret,orgName,newUser,true).then(function(response) {
			if (response && typeof response !== 'string') {
				response.token = token;
				// response.getpassword = response.secret;
				res.json(response);
			} else {
				res.json({
					success: false,
		...
		helper.enrollUser(username,password,orgName,true).then(function(response) {
			if (response && typeof response !== 'string') {
				response.token = token;
				// response.getpassword = response.secret;
				res.json(response);
...
app.post('/channels', function(req, res) {
	...
	helper.enrollUser(req.username,req.password,req.orgname,true).then(function(response) {
		if (response && typeof response !== 'string') {
...
app.get('/channels/:channelName/chaincodes/:chaincodeName', function(req, res) {
	...
	query.queryChaincode(peer, channelName, chaincodeName, args, fcn, req.username,req.password,  req.orgname)
	.then(function(message) {
		res.send(message);
	});
});
...
app.get('/channels/:channelName/blocks/:blockId', function(req, res) {
	...
	query.getBlockByNumber(peer, blockId, req.username, req.password, req.orgname)
		.then(function(message) {
			res.send(message);
		});
});
...
app.get('/channels/:channelName/transactions/:trxnId', function(req, res) {
	...
	query.getTransactionByID(peer, trxnId, req.username, req.password, req.orgname)
		.then(function(message) {
			res.send(message);
		});
});
...
app.get('/channels/:channelName/blocks', function(req, res) {
	...
	query.getBlockByHash(peer, hash, req.username,req.password,  req.orgname).then(
		function(message) {
			res.send(message);
		});
});
...
app.get('/channels/:channelName', function(req, res) {
	...
	query.getChainInfo(peer, req.username,req.password,  req.orgname).then(
		function(message) {
			res.send(message);
		});
});
...
app.get('/channels', function(req, res) {
	...
	query.getChannels(peer, req.username,req.password, req.orgname)
	.then(function(
		message) {
		res.send(message);
	});
