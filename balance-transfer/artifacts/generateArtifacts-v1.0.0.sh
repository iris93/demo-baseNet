...
function replacePrivateKey () {
  ...
  scp fabric-ca-server/fabric-ca-server-config.yaml crypto-config/peerOrganizations/org3.example.com/ca/
  cd crypto-config/peerOrganizations/org3.example.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd $CURRENT_DIR
  sed $OPTS "s/CA3_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose.yaml

  scp fabric-ca-server/fabric-ca-server-config.yaml crypto-config/peerOrganizations/org4.example.com/ca/
